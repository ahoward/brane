...
103. Bound query parameters - db.run(script, params) with $name placeholders - sidestep escaping entirely and round-trip quotes, backslashes and newlines correctly.
104. Every code path that interpolates a user-supplied string into a Cozo query must use one of the two correct approaches. SQL-style doubling silently rejects any value containing an apostrophe.
105. The failure surfaces as: The query parser has encountered unexpected input / end of input at N..N
106. CozoDB string literals use backslash escapes, not SQL-style doubling. 'it''s' is a parse error; 'it\'s' is correct.
107. The bug predates claims and was unreachable until claims existed, because no rule body had ever contained a quote character.
108. It interpolates rule name, description and body into single-quoted Cozo literals using SQL-style doubling (.replace(/'/g, "''")), which CozoDB rejects.
109. The handler lives at src/handlers/mind/rules/create.ts
110. Shipping the claims feature REQUIRES correcting this handler. A user-defined rule that joins *claims must contain the literal 'concept', so claims makes the latent bug reachable. Regenerating claims without fixing rules/create leaves a failing system.
111. A rule whose body contains a single-quoted literal must be creatable and queryable end to end.
