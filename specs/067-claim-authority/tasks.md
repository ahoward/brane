...
- [x] **T001** [P] Write `tests/mind/claims/run` + `data/NN-*/{params,result}.json` covering US1–US4 and US7:
  ...
  relation.
- [x] **T002** [P] Write `tests/mind/authorities/run` + fixtures covering US5: seeded defaults on fresh
...
- [x] **T003** ⛔ **Human-approved fixture reconciliation** (research D11). Update the locked fixtures that
...
- [x] **T004** [P] [US6] Extend `tests/mind/rules/`: `contradictions` returns a concept with competing
...
- [x] **T005** [P] Extend the verify suite: `contradictions` participates in `/mind/verify` like any other
  rule, and fires even when the contradiction resolves cleanly by rank (spec US6, research D7).
- [x] **T006** [P] Cascade tests: deleting a concept removes its claims **and the claims on the edges that
...
- [x] **T007** Migration test (SC-006). Mechanism, since the binary ships at 1.13.0 and a RocksDB fixture
...
- [x] **T008** SC-001 end-to-end: the refund 30/14/45 scenario from quickstart.md — three claims written,
...
- [x] **T010** [FND] Add `claims` and `authorities` relations to `SCHEMA_QUERIES` in
  `src/handlers/mind/init.ts`, exactly as declared in data-model.md (all-key `claims`, `=>`-keyed
  `authorities`).
- [x] **T011** [FND] Seed the five default authority tiers (research D6) on init.
- [x] **T012** [FND] Add the `contradictions` built-in rule to `BUILTIN_RULES` in `init.ts`.
- [x] **T013** [FND] Bump `SCHEMA_VERSION` to `1.13.0` in `init.ts`.
- [x] **T014** [FND] Add the v1.12.0 → v1.13.0 migration in `src/lib/migrate.ts`: both relations, the five
  tiers, the `contradictions` rule. Bump `LATEST_VERSION`.
- [x] **T015** [FND] Add `"contradictions"` to `BUILTIN_RULE_NAMES` in `src/lib/mind.ts`.
- [x] **T016** [FND] Verify init-vs-migrate parity: a fresh v1.13.0 db and a migrated v1.12.0 db are
...
- [x] **T017** [FND] Create `src/lib/claims.ts` with constants: `SUBJECT_TYPES`,
...
- [x] **T018** [FND] `get_next_claim_id(db)` — same pattern as `get_next_annotation_id`.
- [x] **T019** [FND] `authority_exists(db, name)` and `get_authorities(db)`.
- [x] **T020** [FND] `edge_exists(db, id)` in `src/lib/mind.ts` (mirrors `concept_exists`);
  `subject_exists(db, subject_type, subject_id)` in `claims.ts` dispatching to it.
- [x] **T021** [FND] `normalize_claim_fields(params)` — trim `predicate`/`assertion`/`authority`/`source`
  at the boundary so every downstream comparison is exact equality (FR-016b, research D3).
- [x] **T022** [FND] `find_claim(db, tuple)` — the 5-tuple idempotency lookup on stored (trimmed) values.
- [x] **T023** [FND] `fetch_claims_with_rank(db, filters)` — single query joining `claims` → `authorities`.
- [x] **T024** [FND] `group_conflicts(rows)` — grouping, exact assertion comparison, rank-desc/id-asc
  ordering, `resolution`, `unresolved` tie flag (research D5).
- [x] **T025** [FND] `cascade_claims(db, subject_type, ids)` — the single cascade path used by all four
...
- [x] **T026** [US1] `src/handlers/mind/claims/create.ts` — guard-early validation per contracts, trim via
  T021, idempotent insert, `created` flag.
- [x] **T027** [P] [US1] `src/handlers/mind/claims/get.ts` — by ID, with joined rank.
- [x] **T028** [US2] [US4] `src/handlers/mind/claims/list.ts` — filters, `limit`, `resolve` projection,
  rank-desc/id-asc ordering, empty → count 0.
- [x] **T029** [US3] `src/handlers/mind/claims/conflicts.ts` — groups via `group_conflicts`, filters, no
  truncation.
- [x] **T030** [P] [US7] `src/handlers/mind/claims/delete.ts` — read-then-remove by ID, not-found error.

---

## Phase 5: Cascades (US7, FR-015)

- [x] **T031** [US7] `src/handlers/mind/concepts/delete.ts` — call `cascade_claims` for the concept **and
  for the edge IDs the handler already collects to `:rm`** (research D8 trap 1). Report
  `cascade.claims_removed`.
- [x] **T032** [P] [US7] `src/handlers/mind/edges/delete.ts` — `cascade_claims` for the edge; report
  `claims_removed`.
- [x] **T033** [US7] `src/handlers/mind/prune.ts` — `cascade_claims` for pruned concepts and edges
  (research D8 trap 2).
- [x] **T034** [US7] `src/handlers/calabi/extract.ts` — `cascade_claims` on the re-extraction delete path.

---

## Phase 6: Handlers — Authorities (US5)

- [x] **T035** [P] [US5] `src/handlers/mind/authorities/list.ts` — rank-desc, name-asc.
- [x] **T036** [US5] `src/handlers/mind/authorities/create.ts` — upsert, rank validation, `created` flag,
  **description preserved when omitted on update**.
- [x] **T037** [US5] `src/handlers/mind/authorities/delete.ts` — refuse with `conflict` when referenced.

---

## Phase 7: Registration & CLI

- [x] **T038** Register all 8 paths in `src/index.ts`.
- [x] **T039** [P] `src/cli/commands/claim.ts` — `create|list|get|delete|conflicts`.
- [x] **T040** [P] `src/cli/commands/authority.ts` — `list|create|delete`.
- [x] **T041** Wire both into `src/cli/main.ts` under `admin` plus the backward-compat top level, matching
...
- [x] **T042** Run `bun test`. Loop until green. If stuck after good-faith effort → ⛔ human checkpoint.
- [x] **T043** Confirm no behavioral regressions across concepts, edges, provenance, annotations,
  episodes, rules, verify, lenses (SC-005). The only fixture diffs should be the T003 set.
- [x] **T044** [P] Update `CLAUDE.md` Recent Changes + the Spec Machine table status for `067`.
- [x] **T045** [P] Mark `067-claim-authority` complete in `dna/product/ROADMAP.md` and move it to the
  Completed table.
- [x] **T046** Comment the verdict on #113 and note what #114 inherits: the `observation` tier, claim
