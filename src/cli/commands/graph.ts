...
export const graph = defineCommand({
  ...
  subCommands: {
    ...
    neighbors: defineCommand({
      ...
      async run({ args }) {
        const result = await sys.call("/graph/neighbors", {
          id: parseInt(args.id),
        })
    ...
    viz: defineCommand({
      ...
      args: {
        ...
        center: { type: "string", alias: "c", description: "Center visualization on concept ID" },
      ...
      async run({ args }) {
        ...
        if (args.center) params.center = parseInt(args.center)
